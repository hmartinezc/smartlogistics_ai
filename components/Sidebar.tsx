...
const Sidebar: React.FC<SidebarProps> = ({
  ...
  const selectedAgencyLabel =
    currentAgencyId === 'GLOBAL' ? 'Vista Global' : (currentAgency?.name ?? 'Agencia');
  const selectedAgencyDescription =
    currentAgencyId === 'GLOBAL'
      ? 'Todas las agencias'
      : currentAgency?.isActive === false
        ? 'Agencia suspendida'
        : 'Agencia activa';
  ...
  const filteredAgencies = isSearchingAgency
    ? availableAgencies.filter((agency) => {
        ...
        return (
          normalizedName.includes(normalizedAgencySearch) ||
  ...
  return (
    <>
      ...
      >
        {/* Brand Header */}
        <div
          className={`border-b border-slate-100 dark:border-slate-800 ${showLabels ? 'p-6' : 'p-4'}`}
        >
          <div className={`flex items-center ${showLabels ? 'gap-3' : 'justify-center'}`}>
            ...
            {showLabels && (
              <div className="overflow-hidden">
                <h1 className="text-lg font-bold text-slate-900 dark:text-white leading-tight tracking-tight whitespace-nowrap">
                  Smart Invoice
                </h1>
                <p className="text-[10px] text-indigo-500 font-bold uppercase tracking-wider whitespace-nowrap">
                  AI Platform v2.0
                </p>
              </div>
            )}
            ...
            <>
              ...
              <div className="relative" ref={agencyMenuRef}>
                ...
                >
                  ...
                  <ChevronDown
                    className={`h-4 w-4 shrink-0 text-slate-400 transition-transform duration-200 ${isAgencyMenuOpen ? 'rotate-180 text-indigo-500' : 'group-hover:text-slate-600 dark:group-hover:text-slate-200'}`}
                  />
                </button>
                  ...
                  <div className="absolute left-0 right-0 z-50 mt-2 overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-2xl shadow-slate-200/80 dark:border-slate-700 dark:bg-slate-900 dark:shadow-black/40">
                    ...
                    <div className="max-h-64 space-y-1 overflow-auto p-2" role="listbox">
                      {isAdmin && !isSearchingAgency && (
                        ...
                        >
                          <div
                            className={`flex h-9 w-9 shrink-0 items-center justify-center rounded-lg ${currentAgencyId === 'GLOBAL' ? 'bg-white text-indigo-600 shadow-sm dark:bg-slate-900 dark:text-indigo-300' : 'bg-slate-100 text-slate-500 dark:bg-slate-800 dark:text-slate-400'}`}
                          >
                            <Globe className="h-4 w-4" />
                          </div>
                          <div className="min-w-0 flex-1">
                        ...
                        return (
                          ...
                          >
                            ...
                            <div className="min-w-0 flex-1">
                              ...
                              <p
                                className={`text-xs ${agency.isActive ? 'text-slate-400 dark:text-slate-500' : 'text-rose-500 dark:text-rose-300'}`}
                              >
        ...
        {/* Navigation Menu */}
        <div
          className={`sidebar-menu-scroll flex-1 min-h-0 py-4 space-y-1.5 overflow-y-auto ${showLabels ? 'pl-4 pr-3' : 'sidebar-menu-scroll--collapsed px-0 flex flex-col items-center'}`}
        >
          {showLabels && (
            <p className="px-2 text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-2 mt-2">
              Navegación
            </p>
          )}
          ...
          {menuItems.map((item) => {
            ...
            return (
              ...
              >
                <item.icon
                  className={`w-5 h-5 shrink-0 ${isActive ? 'text-white' : 'text-slate-400 group-hover:text-indigo-600 dark:group-hover:text-indigo-400'}`}
                />
                {showLabels && (
                  <div className="flex-1 min-w-0">
                    <div className="font-semibold text-sm truncate">{item.label}</div>
                    <div
                      className={`text-[10px] truncate ${isActive ? 'text-indigo-200' : 'text-slate-400'}`}
                    >
                      {item.desc}
                    </div>
                  </div>
        ...
        <div
          className={`border-t border-slate-100 dark:border-slate-800 bg-slate-50 dark:bg-slate-900/50 ${showLabels ? 'p-4' : 'p-3'}`}
        >
          {showLabels ? (
            <>
              <div className="flex items-center justify-between mb-4 bg-white dark:bg-slate-800 p-3 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm">
                <div className="flex items-center gap-2 overflow-hidden">
                  <div
                    className={`w-8 h-8 rounded-lg flex items-center justify-center text-white font-bold text-xs shrink-0 ${userRole === 'ADMIN' ? 'bg-purple-600' : 'bg-indigo-500'}`}
                  >
            ...
            <div className="flex flex-col items-center gap-2">
              ...
              <button
                onClick={onLogout}
                onMouseEnter={(event) =>
                  openTooltip(event, 'Cerrar Sesión', 'Salir de la plataforma')
                }
                onMouseLeave={closeTooltip}
        ...
        <button
          ...
          onMouseEnter={(event) =>
            openTooltip(
              ...
              isExpanded ? 'Reducir a iconos' : 'Ver nombres y descripciones',
            )
          }
          onMouseLeave={closeTooltip}
