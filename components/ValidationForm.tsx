...
const ValidationForm: React.FC<ValidationFormProps> = ({ data: initialData, onSave, onCancel }) => {
  ...
  useEffect(() => {
    const newTotalPieces = formData.lineItems.reduce(
          ...
          <div className="flex items-center gap-4 text-xs">
            {isPiecesMismatch && (
              <span className="flex items-center gap-1 text-rose-700 font-bold">
                <AlertCircle className="w-3 h-3" /> Error Piezas (Suma: {calculatedPieces} vs Doc:{' '}
                {formData.totalPieces})
              </span>
            )}
            {isEqMismatch && (
              <span className="flex items-center gap-1 text-amber-700 font-bold">
                <AlertCircle className="w-3 h-3" /> Error EQ (Calc: {calculatedEq.toFixed(2)} vs
                Doc: {formData.totalEq})
              </span>
            )}
          </div>
          <span className="text-[10px] text-rose-500 uppercase font-bold tracking-wide">
      ...
      <div className="flex-1 overflow-y-auto bg-slate-50/50 p-4 scrollbar-thin scrollbar-thumb-slate-300">
        ...
        <div className="bg-white rounded-lg border border-slate-200 p-4 mb-4 shadow-sm">
          ...
          >
            ...
            <ChevronDown
              className={`w-4 h-4 text-slate-400 transition-transform ${showHeaderDetails ? 'rotate-180' : ''}`}
            />
          </div>
        ...
        >
          ...
          <div className="bg-slate-50 px-4 py-2 border-b border-slate-200 flex justify-between items-center text-xs">
            ...
            <div className="flex gap-4 font-mono items-center">
              ...
              <div
                className={`flex items-center gap-2 ${isPiecesMismatch ? 'text-rose-600' : 'text-slate-500'}`}
              >
                <span>Pcs:</span>
                <input
                  type="number"
              ...
              <div
                className={`flex items-center gap-2 ${isEqMismatch ? 'text-amber-600' : 'text-slate-500'}`}
              >
                <span>EQ:</span>
                <input
                  type="number"
            ...
            <table className="w-full text-xs text-left">
                ...
                {formData.lineItems.map((item, index) => (
                  <tr key={index} className="hover:bg-indigo-50/30 transition-colors group">
                    <td className="p-1 px-3 align-top">
                      <input
                        value={item.boxType}
                        onChange={(e) => handleItemChange(index, 'boxType', e.target.value)}
                        className="w-full font-bold text-indigo-600 bg-transparent outline-none"
                      />
                    </td>
                    <td className="p-1 px-3 align-top">
                      <input
                    ...
                    <td className="p-1 px-3 bg-slate-50/30 align-top">
                      <input
                        ...
                        className="w-full text-right font-bold text-emerald-600 bg-transparent outline-none"
                      />
                    </td>
                  </tr>
                ))}
              ...
              <tfoot className="bg-slate-100 border-t border-slate-200 font-bold text-slate-700">
                <tr>
                  ...
                  {/* Sum Total Pieces */}
                  <td
                    className={`px-3 py-2 text-center border-t-2 ${isPiecesMismatch ? 'border-rose-400 bg-rose-50 text-rose-700' : 'border-indigo-300 text-indigo-700'}`}
                  >
                    {calculatedPieces}
                  </td>
                  ...
                  {/* Sum Total EQ */}
                  <td
                    className={`px-3 py-2 text-center border-t-2 ${isEqMismatch ? 'border-amber-400 bg-amber-50 text-amber-700' : 'border-indigo-300 text-indigo-700'}`}
                  >
