...
const sortMatches = (items: ProductMatchCatalogItem[]) =>
...
const ProductMatchCatalog: React.FC<ProductMatchCatalogProps> = ({
  currentAgencyId,
  currentAgency,
}) => {
  ...
  const loadCatalog = useCallback(
    async ({ force = false }: LoadCatalogOptions = {}) => {
  ...
  const filteredItems = useMemo(() => {
    ...
    return items.filter((item) =>
      [item.product, item.clientProductCode, item.productMatch, item.htsMatch]
        .join(' ')
        .toLowerCase()
  ...
  const matchedHtsCount = useMemo(
    () => items.filter((item) => item.htsMatch.trim()).length,
    [items],
  );
  const codedProductsCount = useMemo(
    () => items.filter((item) => item.clientProductCode.trim()).length,
    [items],
  );

  const handleDraftChange =
    (field: keyof ProductMatchDraft) => (event: React.ChangeEvent<HTMLInputElement>) => {
      setDraft((prev) => ({
        ...prev,
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    try {
      ...
      setInfoMessage(
        editingId ? 'Match actualizado correctamente.' : 'Match creado correctamente.',
      );
      resetForm();
    } catch (error) {
      setErrorMessage(
  ...
  const handleBootstrapFromMaster = async () => {
    ...
    try {
      ...
      setInfoMessage(
        `Se cargaron ${result.insertedCount} equivalencias base desde la matriz maestra. Las filas idénticas repetidas no se duplican.${repeatedSuffix}`,
      );
    } catch (error) {
      setErrorMessage(
        error instanceof ApiError ? error.message : 'No fue posible cargar el catálogo base.',
      );
    } finally {
  ...
  const handleDownloadTemplate = async () => {
    ...
    try {
      ...
      setInfoMessage(
        'Plantilla descargada correctamente. Ábrela en Excel, llénala y luego impórtala con el botón "Importar Excel".',
      );
    } catch (error) {
      setErrorMessage(
        error instanceof ApiError ? error.message : 'No fue posible descargar la plantilla.',
      );
    }
  };
  ...
  const handleImportFile = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!fileName.endsWith('.xlsx') && !fileName.endsWith('.csv')) {
      setErrorMessage(
        'El archivo debe ser un Excel (.xlsx) o CSV. Descarga la plantilla para obtener el formato correcto.',
      );
      if (fileInputRef.current) {
        fileInputRef.current.value = '';
      }
    ...
    try {
      ...
      setErrorMessage(
        error instanceof ApiError ? error.message : 'No fue posible importar el archivo.',
      );
    } finally {
      ...
      <div className="p-8 max-w-5xl mx-auto animate-in fade-in slide-in-from-bottom-4 duration-500">
        <div className="rounded-[28px] border border-dashed border-slate-300 bg-white/90 p-10 text-center shadow-sm dark:border-slate-700 dark:bg-slate-900/70">
          ...
          <h2 className="text-3xl font-bold tracking-tight text-slate-900 dark:text-white">
            Selecciona una agencia para gestionar su catálogo
          </h2>
          <p className="mx-auto mt-3 max-w-2xl text-sm leading-6 text-slate-500 dark:text-slate-400">
            El catálogo Match Productos es específico por agencia. En contexto GLOBAL solo se
            muestra este estado guiado para evitar cambios fuera de una agencia concreta.
          </p>
        </div>
      </div>
    );
      ...
      <section className="overflow-hidden rounded-[28px] border border-slate-200 bg-white shadow-lg shadow-slate-200/40 dark:border-slate-800 dark:bg-slate-900 dark:shadow-none">
        ...
        <div className="grid gap-4 border-b border-slate-200 px-8 py-6 dark:border-slate-800 md:grid-cols-3">
          <div className="rounded-2xl border border-slate-200 bg-slate-50 p-5 dark:border-slate-700 dark:bg-slate-800/60">
            ...
            <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">
              Catálogo cargado para la agencia activa.
            </p>
          </div>
          <div className="rounded-2xl border border-slate-200 bg-slate-50 p-5 dark:border-slate-700 dark:bg-slate-800/60">
            <div className="text-xs font-bold uppercase tracking-[0.2em] text-slate-400">
            ...
            <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">
              Registros con homologación HTS capturada.
            </p>
          </div>
          <div className="rounded-2xl border border-slate-200 bg-slate-50 p-5 dark:border-slate-700 dark:bg-slate-800/60">
            <div className="text-xs font-bold uppercase tracking-[0.2em] text-slate-400">
            ...
            <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">
              Registros con código producto cliente capturado.
            </p>
          </div>
        </div>
        ...
        <div className="px-8 py-6 space-y-6">
                ...
                <div className="absolute right-0 z-30 mt-2 w-[280px] overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-2xl shadow-slate-200/80 dark:border-slate-700 dark:bg-slate-900 dark:shadow-black/40">
                  <div className="border-b border-slate-100 bg-slate-50 px-4 py-3 dark:border-slate-800 dark:bg-slate-950/70">
                    ...
                    <p className="mt-1 text-sm text-slate-600 dark:text-slate-300">
                      Gestiona el catálogo de equivalencias.
                    </p>
                  </div>
          ...
          {(errorMessage || infoMessage) && (
            <div
              className={`rounded-2xl border px-4 py-3 text-sm ${errorMessage ? 'border-rose-200 bg-rose-50 text-rose-700 dark:border-rose-500/20 dark:bg-rose-500/10 dark:text-rose-200' : 'border-emerald-200 bg-emerald-50 text-emerald-700 dark:border-emerald-500/20 dark:bg-emerald-500/10 dark:text-emerald-200'}`}
            >
          ...
          <div className="grid gap-6 2xl:grid-cols-[360px,minmax(0,1fr)]">
            <form
              ...
              className="rounded-[24px] border border-slate-200 bg-slate-50 p-6 shadow-sm dark:border-slate-700 dark:bg-slate-950/40 2xl:self-start"
            >
              <div className="mb-6 flex items-start justify-between gap-3">
                <div>
                  <h2 className="text-xl font-bold text-slate-900 dark:text-white">
                    {editingId ? 'Editar equivalencia' : 'Nueva equivalencia'}
                  </h2>
                  <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">
                    {editingId
                      ? 'Actualiza la referencia que llega desde la factura y su homologación en el sistema del cliente.'
                      : 'Captura cómo llega el producto en la factura y cómo vive ese mismo producto en el sistema del cliente de la agencia.'}
                  </p>
                </div>
              ...
              <div className="space-y-4">
                <div className="rounded-2xl border border-indigo-100 bg-indigo-50 px-4 py-3 text-sm text-indigo-700 dark:border-indigo-500/20 dark:bg-indigo-500/10 dark:text-indigo-200">
                  ...
                  <p className="mt-1 leading-6">
                    <span className="font-medium">Descripción Product</span> es la referencia exacta
                    con la que se hace el match desde cada line item.{' '}
                    <span className="font-medium">Código producto cliente</span> y{' '}
                    <span className="font-medium">Descripción producto cliente</span> representan
                    cómo ese producto existe en el sistema de la agencia.
                  </p>
                </div>
                <div>
                  ...
                  <input
                    value={draft.product}
                    onChange={handleDraftChange('product')}
                    className="w-full rounded-2xl border border-slate-300 bg-white px-4 py-3 text-sm text-slate-800 outline-none transition focus:border-indigo-400 focus:ring-2 focus:ring-indigo-500/20 dark:border-slate-600 dark:bg-slate-900 dark:text-slate-100"
                    placeholder="Valor exacto que llega en el campo Product"
                  />
                  <p className="mt-1.5 text-xs text-slate-400 dark:text-slate-500">
                    Este valor se toma tal como llega en la factura y es la llave funcional del
                    match.
                  </p>
                </div>
                <div>
                  ...
                  />
                  <p className="mt-1.5 text-xs text-slate-400 dark:text-slate-500">
                    Código como existe dentro del sistema del cliente para esta agencia.
                  </p>
                </div>
                <div>
                  <label className="mb-1.5 block text-sm font-medium text-slate-700 dark:text-slate-300">
                  ...
                  <input
                    ...
                    className="w-full rounded-2xl border border-slate-300 bg-white px-4 py-3 text-sm text-slate-800 outline-none transition focus:border-indigo-400 focus:ring-2 focus:ring-indigo-500/20 dark:border-slate-600 dark:bg-slate-900 dark:text-slate-100"
                    placeholder="Nombre o descripción en el sistema del cliente"
                  />
                  <p className="mt-1.5 text-xs text-slate-400 dark:text-slate-500">
                    Esta descripción es la que se exportará como referencia del producto para la
                    agencia.
                  </p>
                </div>
                <div>
                  ...
                  <input
                    ...
                    className="w-full rounded-2xl border border-slate-300 bg-white px-4 py-3 text-sm text-slate-800 outline-none transition focus:border-indigo-400 focus:ring-2 focus:ring-indigo-500/20 dark:border-slate-600 dark:bg-slate-900 dark:text-slate-100"
                    placeholder="Código HTS homologado para exportación"
                  />
                  <p className="mt-1.5 text-xs text-slate-400 dark:text-slate-500">
                    El campo HTS original se mantiene por compatibilidad, pero ya no forma parte del
                    flujo visible de captura.
                  </p>
                </div>
              </div>
              ...
              <div className="flex items-center justify-between border-b border-slate-200 px-6 py-4 dark:border-slate-700">
                <div>
                  <h2 className="text-lg font-bold text-slate-900 dark:text-white">
                    Grilla de equivalencias
                  </h2>
                  <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">{gridSummary}</p>
                </div>
              </div>
                        ...
                        <td colSpan={5} className="px-6 py-16 text-center">
                          <div className="mx-auto max-w-md text-slate-500 dark:text-slate-400">
                            ...
                            <p className="mt-4 text-base font-semibold text-slate-700 dark:text-slate-200">
                              No hay equivalencias para mostrar
                            </p>
                            <p className="mt-2 text-sm leading-6">
                              {items.length === 0
                                ? 'Puedes crear el primer match manualmente o cargar una base inicial desde la tabla maestra del sistema.'
                                : 'Ajusta la búsqueda o limpia el filtro actual para ver más resultados.'}
                            </p>
                            {items.length === 0 && (
                              <p className="mt-4 text-xs leading-5 text-slate-400 dark:text-slate-500">
                                Usa los botones de la barra superior para descargar la plantilla,
                                importar un Excel o cargar el catálogo base desde la matriz maestra
                                del sistema.
                              </p>
                            )}
                          </div>
                      ...
                      paginatedItems.map((item) => {
                        ...
                        return (
                          ...
                          >
                            <td className="relative px-3 py-4 align-top text-sm font-semibold text-slate-900 dark:text-white">
                              <div className="group relative min-w-0">
                                ...
                                >
                                  ...
                                  <p className="mb-1 text-[10px] font-bold uppercase tracking-[0.16em] text-slate-400 dark:text-slate-500">
                                    Descripción Product
                                  </p>
                            ...
                            <td className="break-words px-3 py-4 align-top text-sm text-slate-600 dark:text-slate-300">
