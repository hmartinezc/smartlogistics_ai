...
const BatchProcessor: React.FC<BatchProcessorProps> = ({ files, format, batchId, onComplete }) => {
      ...
      const processItem = async () => {
        while (nextIndex < queue.length) {
          ...
          try {
            ...
            setItems((prev) =>
              prev.map((item) => (item.id === currentItem.id ? successItem : item)),
            );
            ...
            setItems((prev) => prev.map((item) => (item.id === currentItem.id ? errorItem : item)));
          } finally {
            if (!isCancelled) {
              setActiveCount((prev) => prev - 1);
  ...
  const progressPercentage =
    files.length === 0 ? 0 : Math.round((completedCount / files.length) * 100);
    ...
    <div className="w-full max-w-4xl mx-auto p-6 space-y-8 animate-in fade-in duration-500">
      ...
      <div className="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 max-h-[400px] overflow-y-auto">
        {items.map((item) => (
          <div
            key={item.id}
            className="p-4 border-b border-slate-100 dark:border-slate-700 last:border-0 flex items-center justify-between"
          >
            <div className="flex items-center gap-3">
              ...
              <span
                className={`text-sm ${item.status === 'PROCESSING' ? 'font-bold text-indigo-600' : 'text-slate-600'}`}
              >
                {item.fileName}
              </span>
            </div>
