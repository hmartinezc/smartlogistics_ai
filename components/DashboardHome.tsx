...
const DashboardHome: React.FC<DashboardHomeProps> = ({ results, currentAgencyId, currentAgency, currentPlan }) => {
  ...
  const filteredResults = useMemo(() => {
        return results.filter((result) => {
            const matchesAgency = currentAgencyId === 'GLOBAL' ? true : result.agencyId === currentAgencyId;
    ...
    const invoicedAwbs = useMemo(() => buildInvoicedAwbRecords(results, {
        agencyId: currentAgencyId,
        operationDate,
    }), [currentAgencyId, operationDate, results]);

    const bookedAwbs = useMemo(() => getMockBookedAwbs({
        agencyId: currentAgencyId,
        operationDate,
    }, invoicedAwbs), [currentAgencyId, invoicedAwbs, operationDate]);

    const awbSummary = useMemo(() => buildAwbReconciliationRows(bookedAwbs, invoicedAwbs), [bookedAwbs, invoicedAwbs]);
  ...
  const usagePercent = currentAgency && currentPlan ? (currentAgency.currentUsage / currentPlan.limit) * 100 : 0;
  const isOverLimit = currentAgency && currentPlan ? currentAgency.currentUsage > currentPlan.limit : false;

  // Incident Logic: Status Error OR Low Confidence (<75)
    const incidentsCount = awbSummary.filter((row) => row.status !== 'MATCHED').length;

    const pendingDocumentsCount = awbSummary.filter((row) => row.status === 'PENDING_DOCUMENTS').length;
      ...
      <div className="mb-8 flex flex-col md:flex-row justify-between items-start md:items-end gap-4">
            ...
            <div className="bg-white dark:bg-slate-800 p-4 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700 w-full md:w-80">
                ...
                {isOverLimit && (
                    <div className="mt-1 text-[10px] text-red-500 font-medium flex items-center gap-1">
                        <AlertCircle className="w-3 h-3" /> Excedente aplicado: {currentAgency.currentUsage - currentPlan.limit} págs extra
                    </div>
                )}
            </div>
          )}
          ...
          {currentAgencyId === 'GLOBAL' && (
              <div className="bg-indigo-50 dark:bg-indigo-900/20 p-4 rounded-xl border border-indigo-100 dark:border-indigo-800 flex items-center gap-3">
                  ...
                  <div>
                      <p className="text-xs font-bold text-indigo-800 dark:text-indigo-300 uppercase">Modo Administrador</p>
                      <p className="text-xs text-indigo-600 dark:text-indigo-400">Datos agregados de {awbSummary.length} Masters</p>
                  </div>
              </div>
          )}
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
         <div className="bg-white dark:bg-slate-800 p-6 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700 flex flex-col justify-between">
            <div>
               ...
               <h3 className="text-4xl font-bold text-indigo-600 dark:text-indigo-400">{awbSummary.length}</h3>
            </div>
         ...
         <div className="bg-white dark:bg-slate-800 p-6 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700 flex flex-col justify-between">
             <div>
               ...
               <h3 className="text-4xl font-bold text-slate-800 dark:text-white">{filteredResults.length}</h3>
             </div>
             <div className="mt-4 flex items-center gap-2 text-xs text-slate-400">
         ...
         <div className="bg-white dark:bg-slate-800 p-6 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700 flex flex-col justify-between relative overflow-hidden">
             ...
             <div className="relative">
               ...
               <h3 className={`text-4xl font-bold ${incidentsCount > 0 ? 'text-red-600' : 'text-slate-800 dark:text-white'}`}>
                  {incidentsCount}
               </h3>
             </div>
             <div className="mt-4 flex items-center gap-2 text-xs text-slate-400 relative">
               <FileWarning className={`w-4 h-4 ${incidentsCount > 0 ? 'text-red-500' : 'text-slate-300'}`} /> 
            ...
            <div className="mb-8 grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-700 dark:bg-slate-800">
                    <p className="text-xs font-bold uppercase tracking-wide text-slate-400">Estado Operativo</p>
                    <div className="mt-2 text-2xl font-bold text-slate-800 dark:text-white">{pendingDocumentsCount}</div>
                    <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">AWB booked sin documentos facturados o conciliación incompleta en la fecha seleccionada.</p>
                </div>
                <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-700 dark:bg-slate-800">
                    <p className="text-xs font-bold uppercase tracking-wide text-slate-400">Origen Datos Reserva</p>
                    <div className="mt-2 text-2xl font-bold text-indigo-600">MOCK API</div>
                    <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">Hoy las reservas salen de un proveedor mock determinístico; luego aquí entra el endpoint real.</p>
                </div>
            </div>
            ...
            <table className="w-full text-sm text-left border-collapse">
                <thead>
                    ...
                    <tr className="text-xs text-slate-500 uppercase bg-slate-100 dark:bg-slate-900 border-b border-slate-200 dark:border-slate-700">
                        <th rowSpan={2} className="px-6 py-4 border-r border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 sticky left-0 z-10 w-48">Master AWB</th>
                        
                        {/* RESERVA GROUP */}
                        <th colSpan={3} className="px-6 py-2 text-center border-r border-slate-200 dark:border-slate-700 bg-slate-50/80 dark:bg-slate-800/80 text-slate-600 font-bold tracking-wider">
                           Reserva (Booked)
                        </th>
                        
                        {/* FACTURA GROUP */}
                        <th colSpan={3} className="px-6 py-2 text-center border-r border-slate-200 dark:border-slate-700 bg-indigo-50/50 dark:bg-indigo-900/20 text-indigo-700 font-bold tracking-wider">
                           Facturado (Invoiced)
                        </th>
                        
                        <th rowSpan={2} className="px-6 py-4 text-center bg-white dark:bg-slate-800">Status</th>
                    </tr>
                    ...
                    <tr className="text-[10px] text-slate-500 uppercase bg-white dark:bg-slate-800 border-b border-slate-200 dark:border-slate-700">
                        {/* Reserva Columns */}
                        <th className="px-4 py-2 text-right bg-slate-50/50 border-r border-slate-100">Hijas</th>
                        <th className="px-4 py-2 text-right bg-slate-50/50 border-r border-slate-100">Pcs</th>
                        <th className="px-4 py-2 text-right bg-slate-50/50 border-r border-slate-200">EQ Full</th>
                        
                        {/* Factura Columns */}
                        <th className="px-4 py-2 text-right bg-indigo-50/10 border-r border-slate-100">Hijas</th>
                        <th className="px-4 py-2 text-right bg-indigo-50/10 border-r border-slate-100">Pcs</th>
                        <th className="px-4 py-2 text-right bg-indigo-50/10 border-r border-slate-200">EQ Full</th>
                    </tr>
                </thead>
                <tbody className="divide-y divide-slate-100 dark:divide-slate-700">
                    ...
                    awbSummary.map((awb) => {
                        const hasDiff = awb.status === 'DISCREPANCY' || awb.status === 'PARTIAL' || awb.status === 'PENDING_DOCUMENTS';
                        ...
                        return (
                        <tr key={awb.mawb} className="hover:bg-slate-50 dark:hover:bg-slate-700/50 group">
                            ...
                            {/* RESERVA DATA */}
                            <td className={`px-4 py-4 text-right text-slate-500 text-sm border-r border-slate-100 ${diffHijas ? 'text-amber-600 font-bold bg-amber-50/30' : ''}`}>
                                {awb.bookedHijas}
                            </td>
                            <td className={`px-4 py-4 text-right text-slate-500 text-sm border-r border-slate-100 ${diffPieces ? 'text-amber-600 font-bold bg-amber-50/30' : ''}`}>
                                {awb.bookedPieces}
                            </td>
                            <td className={`px-4 py-4 text-right text-slate-500 text-sm border-r border-slate-200 ${diffFulls ? 'text-amber-600 font-bold bg-amber-50/30' : ''}`}>
            ...
            {filteredResults.length === 0 ? (
                ...
                [...filteredResults].reverse().map((item) => (
                    <div key={item.id} className="flex items-center gap-3 p-2 hover:bg-slate-50 rounded transition-colors">
                        ...
                        <span className={`font-bold ${item.status === 'SUCCESS' ? 'text-green-600' : 'text-red-600'}`}>
                            [{item.status}]
                        </span>
