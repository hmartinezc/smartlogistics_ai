...
const AgenciesConfiguration: React.FC<AgenciesConfigurationProps> = ({ 
    agencies, plans, onAddAgency, onUpdateAgency, onDeleteAgency 
}) => {
  // Form State
                ...
                <div className={`rounded-2xl shadow-lg border p-6 sticky top-6 transition-colors duration-300 ${editingId ? 'bg-amber-50 border-amber-200 dark:bg-amber-900/10 dark:border-amber-800' : 'bg-white border-slate-200 dark:bg-slate-800 dark:border-slate-700'}`}>
                    ...
                    <form onSubmit={handleSubmit} className="space-y-5">
                        ...
                        <div className="flex items-center justify-between bg-slate-50 dark:bg-slate-900/50 p-3 rounded-xl border border-slate-100 dark:border-slate-700">
                           <span className="text-sm font-medium text-slate-700 dark:text-slate-300">Estado de Cuenta</span>
                           <button 
                              type="button"
                           ...
                           >
                              <span className={`inline-block h-4 w-4 transform rounded-full bg-white transition transition-transform ${isActive ? 'translate-x-6' : 'translate-x-1'}`} />
                           </button>
                        </div>
                        ...
                        <div>
                            <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Emails Facturación</label>
                            ...
                            <div className="flex flex-wrap gap-2">
                                {emailList.map((email) => (
                                    <span key={email} className="inline-flex items-center gap-1 px-2 py-1 rounded-md bg-slate-100 dark:bg-slate-700 text-xs text-slate-700 dark:text-slate-300 border border-slate-200 dark:border-slate-600">
                                        {email}
                                        <button 
                                            type="button" 
                        ...
                        <div>
                             <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Plan de Suscripción</label>
            ...
            <div className="lg:col-span-2">
                <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700 overflow-hidden">
                    <div className="p-6 border-b border-slate-100 dark:border-slate-700 bg-slate-50 dark:bg-slate-900/50">
                        <h3 className="text-lg font-bold text-slate-800 dark:text-white">Agencias Activas ({agencies.length})</h3>
                    </div>
                                ...
                                {agencies.map(agency => {
                                    ...
                                    return (
                                        <tr key={agency.id} className="hover:bg-slate-50 dark:hover:bg-slate-700/50">
                                            ...
                                            <td className="px-6 py-4">
                                                <div className="font-bold text-slate-800 dark:text-white">{agency.name}</div>
                                                <div className="flex flex-col gap-1 mt-1">
                                                    {agency.emails && agency.emails.length > 0 ? (
                                                        agency.emails.map(email => (
                                                            <span key={email} className="text-[10px] text-slate-500 bg-slate-100 dark:bg-slate-700 px-1.5 py-0.5 rounded w-fit">
                                                                {email}
                                                            </span>
                                                        ))
                                                    ) : (
                                                ...
                                                <div className="text-[10px] text-slate-400 font-mono mt-1">{agency.id}</div>
                                            </td>
                                            <td className="px-6 py-4">
                                            ...
                                            <td className="px-6 py-4 text-center">
                                                ...
                                                <span className="text-slate-400 text-xs"> / {plan?.limit.toLocaleString()}</span>
                                            </td>
